...
def load_view():
    ...
    # create sidebar
    with st.sidebar:
        st.header("PARAMETER VISUALISASI PETA")
        map_type = st.radio('PILIH PETA PERSENTASE PENDUDUK MISKIN (P0) TAHUN 2021 YANG INGIN DITAMPILKAN', (
            'Prediksi P0 Menggunakan Model Ridge Dengan Extracted Features dan Light Features',
            'Prediksi P0 Menggunakan Model LASSO Dengan Extracted Features dan Light Features',
            'Prediksi P0 Menggunakan Model Ridge Dengan Extracted Features',
            'Prediksi P0 Menggunakan Model LASSO Dengan Extracted Features',
            'P0 Berdasarkan Data BPS'
        ))


    # section 1
    st.header('PETA ' + map_type.upper() + " TAHUN 2021")

    # merge shape file and bps data
    merged_data = pd.merge(shp_data, data_complete, on='KAB_KOTA')

    # calculate position
    latitude = merged_data.geometry.centroid.y.mean()
    longitude = merged_data.geometry.centroid.x.mean()

    # Create a Folium map object
    map_y = folium.Map(location=[latitude, longitude], scrollWheelZoom = False, zoom_start = 8)

    # create map based on the selected map type
    if map_type == 'Prediksi P0 Menggunakan Model Ridge Dengan Extracted Features dan Light Features':
        y_column = 'y pred ridge with extracted features, light features'
    elif map_type == 'Prediksi P0 Menggunakan Model LASSO Dengan Extracted Features dan Light Features':
        y_column = 'y pred lasso with extracted features, light features'
    elif map_type == 'Prediksi P0 Menggunakan Model Ridge Dengan Extracted Features':
        y_column = 'y pred ridge with extracted features'
    elif map_type == 'Prediksi P0 Menggunakan Model LASSO Dengan Extracted Features':
        y_column = 'y pred lasso with extracted features'
    else: y_column = 'y actual'

    # Add the poverty index as a choropleth map layer
    Choropleth(geo_data = merged_data, data = merged_data, columns = ['KAB_KOTA', y_column],
